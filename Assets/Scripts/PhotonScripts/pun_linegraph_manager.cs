...
public class pun_linegraph_manager : MonoBehaviour {
	...
	void Update(){
		if (pun_game_controller.showGraph) {
			ShowGraph ();
			pun_game_controller.showGraph = false;
		}
		if (pun_game_controller.addLearningPoint) {
			if (pun_game_controller.playerNo == 1) {
				AddPlayer1Data (pun_move_analysis.learning_point_p1);
				if (Mathf.Abs(pun_move_analysis.learning_point_p1) > highestValue) {
					highestValue = Mathf.Abs(pun_move_analysis.learning_point_p1);
				}
			} else {
				AddPlayer2Data (pun_move_analysis.learning_point_p2);
				if (Mathf.Abs(pun_move_analysis.learning_point_p2) > highestValue) {
					highestValue = Mathf.Abs(pun_move_analysis.learning_point_p2);
				}
			}
			pun_game_controller.addLearningPoint = false;
		}
	}
